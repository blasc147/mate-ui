...
export const headers: Column[] = [
  {
    ...
    Cell: ({ row }) => (
      <div>
        <p className="text-sm text-grey-900">{row.original.name}</p>
        <p className="hidden text-xs text-grey-600 md:block">
  ...
  },
  {
    ...
    Cell: ({ row }) => (
      <Tag
        ...
        variant="status"
        rightIcon={<CheckBadgeIcon className="h-4 w-4 !text-grey-700" />}
      >
        {row.original.status}
      </Tag>
    ),
